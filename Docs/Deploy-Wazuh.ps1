...
'@
    # Install valhallaAPI right before we need it
    try { & python -m pip install --quiet valhallaAPI } catch { Log "valhallaAPI pip failed: $_" "WARN" }
...
# Detect correct service name (WazuhSvc on newer installs, Wazuh on older)
$wazuhSvcName = if (Get-Service -Name "WazuhSvc" -ErrorAction SilentlyContinue) { "WazuhSvc" } else { "Wazuh" }
try {
    Restart-Service -Name $wazuhSvcName -Force -ErrorAction Stop
    Log "Wazuh service restarted ($wazuhSvcName)"
} catch {
    try { Start-Service -Name $wazuhSvcName; Log "Wazuh service started ($wazuhSvcName)" }
    catch { net start $wazuhSvcName 2>&1 | Out-Null; Log "Wazuh started via NET START" }
}
# Set startup type — try both known service names
foreach ($svcName in @("WazuhSvc","Wazuh")) {
    $s = Get-Service -Name $svcName -ErrorAction SilentlyContinue
    if ($s) { Set-Service -Name $svcName -StartupType Automatic -ErrorAction SilentlyContinue; break }
}

$status = (Get-Service -Name $wazuhSvcName -ErrorAction SilentlyContinue).Status
